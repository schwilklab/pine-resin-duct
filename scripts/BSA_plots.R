...
ggsave("../results/ring_width_PMDI.pdf", width=16, height=10, units="cm")
...
ggplot(rw_ss_df, aes(BA, rw, color= subsection)) +
  ...
  prestheme.nogridlines
ggsave("../results/ring_width_BAF.pdf", width=16, height=10, units="cm")
...
ggplot(bytree, aes(subsection, dd)) + geom_boxplot() +
  ylab(expression("Resin duct density ("*cm^-2*")")) +
  xlab("Subsection") +
  prestheme.nogridlines
ggsave("../results/rdd_subsection.pdf", width=14, height=10, units="cm")
...
ggplot(bytree, aes(subsection, dd)) + geom_boxplot() + facet_grid(. ~ age_f) +
  ...
  ylab(expression("Resin duct density ("*cm^-2*")")) +
  prestheme.nogridlines
ggsave("../results/rdd_age_subsection.pdf", width=22, height=10, units="cm")


# rdd by elevation and subsection and age
ggplot(bytree, aes(elev, dd, color=subsection)) + geom_point()+
  facet_grid(. ~ age_f) +
   theme(axis.text.x=element_text(angle=45, hjust=1)) +
   scale_color_manual(values = schwilkcolors, guide_legend(title = "Subsection"))+
  ...
  ylab(expression("Resin duct density ("*cm^-2*")")) +
  prestheme.nogridlines
ggsave("../results/rdd_elev_age_subsection.pdf", width=22, height=10, units="cm")


# rdd by subsection, age and rw
ggplot(bytree, aes(rw, dd, color=subsection)) + geom_point()+
  scale_color_manual(values = schwilkcolors, guide_legend(title = "Subsection"))+
  facet_grid(. ~ age_f) +
  ...
  ylab(expression("Resin duct density ("*cm^-2*")")) +
  prestheme.nogridlines
ggsave("../results/rdd_rw_age_subsection.pdf", width=22, height=10, units="cm")
...
bytree <- mdata %>%  mutate(rw_f = factor(ring_width_detrended > 0, labels=c("Narrow", "Wide"))) %>%
...
#  facet_grid(. ~ rw_f) +
  ...
  ylab(expression("Resin duct density ("*cm^-2*")")) +
  prestheme.nogridlines
ggsave("../results/rdd_elev_rwf_subsection.pdf", width=18, height=10, units="cm")
...
ggplot(bytree, aes(rw, dd, color=subsection)) + geom_point()+
  ...
  ylab(expression("Resin duct density ("*cm^-2*")")) +
  prestheme.nogridlines
ggsave("../results/rdd_rw_elevf_subsection.pdf", width=18, height=10, units="cm")
...
ggplot(byage, aes(age, dd, color=subsection)) + geom_point()+
  scale_color_manual(values = schwilkcolors, guide_legend(title = "Subsection"))+
  facet_grid(. ~ rw_f) +
  geom_smooth(aes(group=tag), method="lm", se=FALSE) +
  ylim(c(0,300)) +
  xlab("ambial age (years)") +
  ylab(expression("Resin duct density ("*cm^-2*")")) +
  prestheme.nogridlines
ggsave("../results/rdd_byage_subsection.pdf", width=18, height=10, units="cm")
